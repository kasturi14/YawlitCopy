...
import { CheckoutServices } from '../models/CheckoutServices';
...
export class ServicesComponent implements OnInit {
  ...
  checkOutObj:CheckoutServices=<any>{};
  processingPayment: boolean;
  selectedOrderObj:Services=<any>{};
  selectedServiceId:string;
  BookingOrder:AdminServiceDetails=<any>{};
  private razor_order:string;
  private razor_payment:string;
  public CustomerKey:string;
  private PartnerKey:string;
  WindowRef: any;
  minDate:string;
  public Glitch:boolean=false;
  private minDateSet=new Date();
  private myDate=new Date();
  spModal:any;

  
  constructor(config: NgbModalConfig, private modalService: NgbModal,private userSave:UserSaveService,private changeRef: ChangeDetectorRef,private paymentService:PaymentsService,private Orderservice:OrderServiceService,private datePipe: DatePipe,private router:Router,private auth:AuthService,private planService:PlansService,private db:AngularFireDatabase) { 
...
resetMess()
{
  ...
  //gets the services for the vehicles selected
  setSedan()
  {
    this.vehicle="Sedan";
...
//gets the services of either type simple or premium based on the slider's value
slider()
{
  ...
  selectedOrder(p)
  {
    ...
    if(this.CustomerKey)
    {
    this.selectedServiceId=p.key; //set the selected Service ID
    this.selectedOrderObj=p;//set the selectedServiceId object selected service's data
  }
  ...
  //initializes the checkOutObj object with the form's data
  finalForm(value)
  {
...
bookservice()
{
    ...
    //the booking details
    this.BookingOrder.Pincode=this.Pincode;
    this.BookingOrder.Time=this.checkOutObj.Time;
    this.BookingOrder.Address=this.appUser.Address;
    this.BookingOrder.Date=this.checkOutObj.Date;
    this.BookingOrder.ServiceType=this.selectedOrderObj.ServiceType;
    ...
    this.BookingOrder.ServiceName=this.selectedOrderObj.ServiceName;
    this.BookingOrder.Saving=this.selectedOrderObj.Saving;
    this.BookingOrder.Vehicle=this.vehicle;
    this.BookingOrder.serviceId=this.selectedServiceId;
    const BookedAt= this.datePipe.transform(new Date(), 'yyyy-MM-dd');
    this.BookingOrder.BookedAt=BookedAt;
}


codService()
  {
    this.spModal.close();//close the payment modal
    this.bookservice();//initialize the booking object
    //some alerts
    ...
    this.BookingOrder.Mode="Cash On Wash";
    this.Orderservice.createCODService(this.BookingOrder,this.CustomerKey);//call the createCODService function from Orders Service
...
initiatePaymentModal(event) {
  ...
  let orderDetails = {
    ...
    notes:{'Plan Name':this.BookingOrder.ServiceName,
            'Vehicle': this.BookingOrder.Vehicle,
            'Start Date': this.BookingOrder.Date,
            'Time': this.BookingOrder.Time,
            }
  }
     ...
     handlePayment(response) {
      ...
      this.paymentService.capturePayment(response)
        .subscribe(res => {
          ...
          this.Orderservice.createService(this.CustomerKey,this.BookingOrder);//calls the createService function of the Order Service
          this.spModal.close();
          this.router.navigate(['/ThankYou']);
...
open(content) {
  this.spModal=this.modalService.open(content, { centered: true});
}
